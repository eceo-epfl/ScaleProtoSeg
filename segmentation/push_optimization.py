"""Computationally optimized code for the pushing mechanism at single-scale used for ADE and COCO"""
...
def compute_distances(
    ...
    Args:
        ppnet (PPNet): Input model to compute the distances.
...
def min_across_dataset(
    ...
    Args:
        ...
        ppnet (PPNet): Input model to compute all distances.
...
def global_min(
    ...
    Args:
        ...
        ppnet (PPNet): Input model to compute the feature mapp.
    ...
    num_scales = 1
    n_prototypes_scale = ppnet.num_prototypes // num_scales
    ppnet.to(device)
    ...
    for p in tqdm(range(ppnet.num_prototypes), desc="Computing Minimums on all Prototypes", total=ppnet.num_prototypes):
        ...
        conv_features = conv_features.view(num_scales, int(C / num_scales), H, W)
        ...
        global_min_fmap_patches.append(
            conv_features[current_scale, :, i : i + 1, j : j + 1].detach().cpu().numpy()
        )  # TODO: Is +1 necessary?
...
def push_prototypes(
    ...
    Args:
        ...
        prototype_network_parallel (PPNet): Model with prototypes.
