"""
Adaptation from train.py to support weight & biases logging and other datasets: ADE20K & COCO-Stuff
"""
...
# Change to from segmentation.em.module_em import PatchClassificationModule for EM dataset.
from segmentation.push import push_prototypes
from segmentation.push_optimization import push_prototypes as push_prototypes_optim
...
def train(
    ...
    """Training script for the ProtoSeg Baseline
    ...
    if not pruned:
        ...
        module = PatchClassificationModule(model_dir=results_dir, ppnet=ppnet, training_phase=1, max_steps=joint_steps)
        ...
        if data_type in ["ade", "coco"]:
            push_prototypes_optim(
                push_dataset,
